...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM2) {
        // Tắt tất cả 4 LED trước khi cập nhật
        HAL_GPIO_WritePin(GPIOA, EN0_Pin|EN1_Pin|EN2_Pin|EN3_Pin, GPIO_PIN_SET);
        // Đặt tất cả segment pins về trạng thái tắt (high) để tránh rò rỉ
        HAL_GPIO_WritePin(GPIOB, SEG0_Pin|SEG1_Pin|SEG2_Pin|SEG3_Pin|SEG4_Pin|SEG5_Pin|SEG6_Pin, GPIO_PIN_SET);

        // Switch between four seven-segment displays
        switch (led_index) {
            case 0:
                display7SEG(1); // LED1 hiển thị số 1
                HAL_GPIO_WritePin(GPIOA, EN0_Pin, GPIO_PIN_RESET); // bật LED1
                HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET); // PA5 sáng
                HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); // DOT tắt
                break;
            case 1:
            ...
            case 2:
                ...
                HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET); // PA5 tắt
                HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET); // DOT ban đầu tắt
                break;
            case 3:
        ...
        led_index = (led_index + 1) % 4; // Chu kỳ qua 4 LED (0->1->2->3->0)

        // Quản lý nháy DOT LEDs khi led_index là 2 hoặc 3
        counter--;
        if (counter <= 0) {
...
static void MX_GPIO_Init(void)
{
    ...
    /* Configure GPIO pins : LED_RED_Pin EN0_Pin EN1_Pin EN2_Pin EN3_Pin GPIO_PIN_4 */
    ...
    /* Configure GPIO pins : SEG0_Pin SEG1_Pin SEG2_Pin SEG3_Pin SEG4_Pin SEG5_Pin SEG6_Pin */
