...
int led_buffer[4] = {1, 2, 3, 0};   // dữ liệu hiển thị trên 4 LED
int dot_counter = 4;                // đếm số lần quét để toggle DOT
/* USER CODE END PV */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM2) {
        ...
        // sau khi quét đủ 4 LED (1 giây) -> toggle DOT
        if (--dot_counter <= 0) {
            dot_counter = 4;
            HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5); // DOT LED ở PA4
        }
    }
}
/* USER CODE END 4 */
...
static void MX_TIM2_Init(void) {
  ...
  htim2.Init.Period = 249;       // 250 ms interrupt
